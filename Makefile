...
FIRECRACKER_ARCHIVE=firecracker-$(FIRECRACKER_VERSION)$(FIRECRACKER_ARCH_SUFFIX).tgz
FIRECRACKER_SHA256=$(shell awk '$$2 == "$(FIRECRACKER_ARCHIVE)" { print $$1 }' hack/FIRECRACKER_SHA256SUMS)
...
ifeq ($(GOARCH),amd64)
	...
	$(DOCKER) build -t $(IMAGE):${IMAGE_DEV_TAG}-$(GOARCH) \
		...
		--build-arg FIRECRACKER_ARCH_SUFFIX=${FIRECRACKER_ARCH_SUFFIX} \
		--build-arg FIRECRACKER_SHA256=${FIRECRACKER_SHA256} bin/$(GOARCH)
